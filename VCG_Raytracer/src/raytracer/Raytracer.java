...
public class Raytracer {
    ...
    public void renderScene() {
        ...
        while (totalSamples.get() < mMaxRecursions) {
            ...
            int batchSize = Math.max(1, height / numThreads);
            for (int rowStart = 0; rowStart < height; rowStart += batchSize) {
                final int start = rowStart;
                final int rowEnd = Math.min(start + batchSize, height);

                tasks.add(() -> {
                    for (int finalY = start; finalY < rowEnd; finalY++) {
                        for (int finalX = 0; finalX < width; finalX++) {
                            ...
                            while (aaSampleIndex < mAntiAliasingSamples) {
                                ...
                                Ray primaryRay = camera.getRay((finalX + offsetX) / width, (finalY + offsetY) / height);
                                RgbColor sampleColor = scatter(primaryRay);

                                int pixelIndex = finalY * width + finalX;
                                ...
                                mBufferedImage.setRGB(finalX, finalY, Tonemapper.ACES(hdrColors[pixelIndex]).getRGB());
                                aaSampleIndex++;
                            }
                        }
                    }
            ...
            Log.print(this, "Rendering: " + totalSamples.get() + " samples, " + (elapsedTime / 1000) + " seconds elapsed.");
        }
    ...
    private RgbColor computeDirectLighting(Intersection intersection, Vec3 viewDir) {
        RgbColor directLighting = intersection.material().getColor(intersection.point(), intersection.normal(), viewDir, mScene.getLights());
    ...
    private RgbColor computeGiSamples(Ray ray, int samples) {
        ...
        for (int sampleCount = 0; sampleCount < samples; sampleCount++) {
            ...
            while (bounceDepth < mGiLevel) {
                ...
                RgbColor giSampleColor = giIntersection.material().getColor(giIntersection.point(), giIntersection.normal(), viewDir, mScene.getLights());
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (closestIntersection != null) {
                    ...
                    pixelColor = closestIntersection.material().getColor(closestIntersection.point(), closestIntersection.normal(), viewDir, mScene.getLights());
    ...
    public int[] generateRaySphereIntersections() {
        ...
        PhongMaterial material = new PhongMaterial(mAmbientLight, RgbColor.WHITE, RgbColor.WHITE, 32.0f);
